...
def doExport():
    try:
        ...
        filename = 'Real Stream favoritos.zip'
...
def extractAll(filename, dp, location):
    ...
    profile = os.path.join(root, 'Real Stream Favoritos')
    ...
    if IMPORT_RESET:
        try:    sfile.rmtree(os.path.join(ROOT, 'Real Stream Favoritos'))
        except: pass
        ...
        for item in zin.infolist():
            ...
            if filename == 'settings.xml':
                ...
                try:
                    ...
                    utils.log(e)
            elif filename.lower().startswith('Real Stream Favoritos'):
                zin.extract(item, root)
